...
export function loadChatLanguage(): string {
  try {
    const v = Platform.OS === "web" ? localStorage.getItem(KEY) : SecureStore.getItem(KEY);
...
export function saveChatLanguage(code: string): void {
  try {
    if (Platform.OS === "web") localStorage.setItem(KEY, code);
    else SecureStore.setItem(KEY, code);
  } catch {
    /* not remembered */
  }
}
